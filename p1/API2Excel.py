...
def getXML():
    ...
    val = {'devid': devid, 'appid': appid, 'appkey': appkey, 'date': date, 'perpage': perpage, 'page': page}
...
Label(root, text=u'每页最大数量:', width=10).grid(row=2, column=0)
Label(root, text=u'页数:', width=10).grid(row=2, column=2)
...
perpage_entry = Entry(root, width=20, textvariable=e_perpage)
page_entry = Entry(root, width=20, textvariable=e_page)
...
perpage_entry.grid(row=2, column=1)
page_entry.grid(row=2, column=3)
