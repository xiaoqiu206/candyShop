...
#mysql的配置
mysql_host = u"localhost"   #IP
mysql_port = u"3306"   #端口
mysql_database = u"DX"  #库名
mode3_table = u"mode3"      #表名
mode6_table = u"mode6"       #表名
mysql_user = u"root"      #用户名
mysql_password = u"root"   #密码

#sqlsever的配置
mssql_host = u"192.168.1.4"    #IP
mssql_port = u"1433"     #端口
...
def getMssql3Data():
    mode3_sql = """
        SELECT     hjyw_jcjg.JCFFID, hjyw_jcjg.JCZT, hjyw_jcjg.HCPFXZ, hjyw_jcjg.HCPFJG, hjyw_jcjg.HCPFPD, hjyw_jcjg.COPFXZ, hjyw_jcjg.COPFJG, hjyw_jcjg.COPFPD, 
                      ...
                      hjyw_gcsj.YDZGCSJ, hjyw_gcsj.GXSXSGCSJ, hjyw_gcsj.ID
...
WHERE     (hjyw_jcjg.JCFFID = 3) AND (hjyw_jcjg.JCZT = 1) AND (hjyw_jcjg.COPFJG > '5') AND (hjyw_jcjg.NOXPFJG > '1')
    ...
    if mode3_maxid is None:
        ...
        mssqlCursor.execute(mode3_sql + ' and hjyw_gcsj.ID > %s',(mode3_maxid,))
...
WHERE     (hjyw_jcjg.JCFFID = 6) AND (hjyw_jcjg.JCZT = 1) AND (hjyw_jcjg.PFJG100 > '0.8')
    ...
    if mode6_maxid is None:
        ...
        mssqlCursor.execute(mode3_sql + ' and hjyw_gcsj.ID > %s',(mode6_maxid,))
    dataList = mssqlCursor.fetchall()
    return dataList
